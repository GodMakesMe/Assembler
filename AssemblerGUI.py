...
class Assembler:
    def __init__(self,a=1, textfiledir = "", outputdir = ""):
        self.root = tk.Tk()
        self.a = a
        ...
        if instype=="R":
            ...
            try:
                ...
                if code[2] not in self.labels:
                    ...
                    immval += self.labels[code[2]] - line + self.emptylinecheck(line)-self.emptylinecheck(self.labels[code[2]])+1
                immval *= 4
    ...
    def emptylinecheck(self, line):
        fullcode = self.commentsremover(self)
        fullcode = fullcode.split("\n")[:line]
        emptlylinecount = 0
        for i in range(len(fullcode)):
            if fullcode[i] == "":
...
if __name__ == "__main__":
    ...
    asmbly = Assembler(a=1)      # incase we want to have the GUI version
